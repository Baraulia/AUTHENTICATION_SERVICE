...
func (u *UserPostgres) CreateUser(user *model.User) (*model.User, error) {
	...
	if err := row.Scan(&user.ID, &user.Email, &user.Password, &user.CreatedAt); err != nil {
		...
		return nil, fmt.Errorf("create user: error while scanning for user:%s", err)
	}
	//send user password
...
func (u *UserPostgres) GetUserByEmail(email string) (model.User, error) {
	...
	var User model.User
	result, err := db.Query("SELECT id, email, password, created_at FROM users WHERE email = $1", email)
	if err != nil {
		return User, err
	}
